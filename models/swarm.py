...
class EvolutionarySwarm:
    ...
    def __init__(self, dataset_config, initial_population_size):
        ...
        self.subpopulations = [
            Subpopulation([NanoModel(dataset_config) for _ in range(initial_population_size // NUM_SUBPOPULATIONS)], 
    ...
    def evolve(self, data, targets):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(subpop.adapt, self.environment, data, targets) for subpop in self.subpopulations]
    ...
    def bottleneck_effect(self):
        ...
        """
        for subpop in self.subpopulations:
            subpop.models = subpop.models[:len(subpop.models)//5]  # Drastyczne zmniejszenie populacji
    ...
    def load_checkpoint(self, path):
        ...
        for i, subpop_data in enumerate(checkpoint['subpopulations']):
            ...
            self.subpopulations[i].models = [
                NanoModel.from_json(model_json) if isinstance(model_json, str) else SymbioticPair.from_json(model_json)
    ...
    def parallel_evolve(self, data, targets, num_processes=NUM_PROCESSES):
        ...
        try:
            with concurrent.futures.ProcessPoolExecutor(max_workers=num_processes) as executor:
                futures = [executor.submit(self.evolve, data, targets) for _ in range(num_processes)]
                concurrent.futures.wait(futures)
